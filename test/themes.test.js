...
describe("themes — Registry", () => {
  it("drei kaufbare Starter-Themes mit allen sechs Elementen", () => {
    for (const id of ["sunset", "lofi", "kaiju"]) {
      const t = THEME_DEFS[id];
      expect(t.kind).toBe("buy");
      expect(t.els).toEqual(["deck", "bf", "frameGlow", "holoSwipe", "hologrid", "laser"]);
      expect(isBuyTheme(t)).toBe(true);
    }
  });
  it("Element-Katalog deckt genau die sechs Element-Keys ab; vier davon sind Animationen", () => {
    expect(ELEMENT_DEFS.map((e) => e.key)).toEqual(["deck", "bf", "frameGlow", "holoSwipe", "hologrid", "laser"]);
    expect(FX_KEYS).toEqual(["frameGlow", "holoSwipe", "hologrid", "laser"]);
    expect(FX_OPTION_KEY.frameGlow).toBe("fxFrameGlow");
    expect(FX_OPTION_KEY.laser).toBe("fxLaser");
  });
...
describe("themes — Kauf-Ökonomie", () => {
  ...
  it("buyAllInfo: Rabatt durch bereits besessene Elemente", () => {
    ...
    expect(info.remainingCount).toBe(4);
    expect(info.cost).toBe(4);
  });
  it("buyAllForTheme kauft alle offenen Elemente (alles-oder-nichts bei zu wenig SP)", () => {
    const t = THEME_DEFS.sunset;
    expect(buyAllForTheme(prof({ stichPoints: 5 }), t)).toEqual(expect.objectContaining({ stichPoints: 5 })); // 5 < 6 → No-op
    const done = buyAllForTheme(prof({ stichPoints: 7 }), t);
