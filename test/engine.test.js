...
describe("resolveTrick — Grundausgänge (V2: ohne Leben)", () => {
  ...
  describe("lastTrick.hitTypes — Score-Float-Identitäten (mehrfach möglich)", () => {
    // §6.2: die Pflanze-Identität hängt am Zustand BLÜHEND (card.bloom), nicht mehr am Kartenwert — die Wertachse ist mit dem Rework gestrichen.
    const greenDeck = (v, bloom = true) => Array.from({ length: 40 }, (_, i) => ({ id: `G${i}`, suit: "G", baseRank: v, value: v, green: true, bloom }));
    ...
    it("Pflanze: Sieg mit einer blühenden Karte", () => {
    ...
    it("normaler Sieg ohne Auslöser → leer; unter den Schwellen auch leer", () => {
      ...
      expect(resolveTrick(scenario(0, 0, { deck: greenDeck(10, false), oppDeck: constDeck(0) }), rng).lastTrick.hitTypes).toEqual([]); // grün, aber noch nicht blühend
    ...
    it("mehrere zugleich: voll gewachsene grüne, voll ionisierte Karte bei voller Hitze → alle drei", () => {
      const deck = Array.from({ length: 40 }, (_, i) => ({ id: `M${i}`, suit: "G", baseRank: 11, value: 11, green: true, bloom: true, ionStacks: 5 }));
