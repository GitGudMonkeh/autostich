...
| --- | --- | --- | --- |
| `skill-rework.md` | **living** | 2026-09-04 | Skill rework on the `exp` playground: 15 skills per faction, four rarity tiers, passives. Declares itself a *lebendes Dokument*; filled faction by faction with the owner. Written in German by declared exception (product content) |
...
| `haltungen-fraktion.md` | **living** / design | 2026-09-22 | Fifth faction, the stance design the owner proposed. Mechanism and the four stance passives are set; two of five skill lines are filled. Declares *"Keine Zahlen sind tariert, nichts ist gemessen."* Written in German by declared exception (product content). Carries the rejected options with their reasons, the arithmetic behind the switch cadence, and the checked name collisions |
...
| `statistik-redesign.md` | **live** | 2026-08-24 | Stats screen rebuild (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; the content scrolls, not the window. Carries the worked example for design-sprache's *Wenn die Anzahl schwankt*. Also covers the run-detail window and the counting pattern behind its build panel |
| `bestenliste-redesign.md` | **live** | 2026-08-24 | Leaderboard rebuild (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; the screen serves two entry points (look up vs. play) and gets two head variants rather than two screens. Mostly a catch-up: it still carries the tree's nav column from before two decisions |
| `erststart-redesign.md` | **live** | 2026-08-24 | First-start screen -- name and language (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; measured with an empty store at 1280x720, 1536x791 and 1400x700. Carries the one named exception to the single-glow rule |
| `werkstatt-redesign.md` | **live** | 2026-08-24 | Deck-Werkstatt rebuild (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; the catalogue keeps its column widths and the card back becomes the hero image instead. Carries the measured rejection of wider tiles, and the challenge tab's condition moves off the tile into the permanently visible preview |
| `battlefield-redesign.md` | **live** | 2026-08-24 | Run screen instruments (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; the stage itself (`#buehne`) is explicitly out of scope. Carries the scope ruling for non-overlay screens: `design-sprache.md` §1 applies, §2 and §4 do not |
| `upgrade-baum-redesign.md` | **live** | 2026-08-24 | Upgrade tree rebuild (≥ 1280 px), all four steps. Declares *"freigegeben, Umsetzung ausstehend"* — approved, implementation pending. Design side only; every figure measured at 1280x720, 1400x700 and 1536x791, never against 1920. Reads `design-sprache.md` first |
...
## Outward-facing text and media

Not player-visible text — nothing here is rendered by the game, and none of it belongs in the i18n
catalogs.

| File | Status | Date | What it is |
| --- | --- | --- | --- |
| `pitch.md` | **live** | 2026-08-28 | The one-sentence pitch in all four project languages, with the short cuts and the genre line. Owner-approved wording; declares itself not player-visible |
| `soundtrack-downloads.md` | **generated** | 2026-08-28 | Download link per soundtrack track, grouped by intensity tier. Generated by `maintenance/music-links.mjs` from `src/ui/music.js` and `media/music/` — regenerate with `npm run music:links`. **Never hand-edit** |
