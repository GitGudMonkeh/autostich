...
| `statistik-redesign.md` | **live** | 2026-08-24 | Stats screen rebuild (>= 1280 px). Declares *"freigegeben, Umsetzung ausstehend"* -- approved, implementation pending. Design side only; the content scrolls, not the window. Carries the worked example for design-sprache's *Wenn die Anzahl schwankt*. The run-detail window is explicitly NOT in scope and gets its own pass |
