...
## Round 7 (owner playtest after round 6) — COLLECTING

| # | Item | Change |
| --- | --- | --- |
| Y1 | **NEW hint + lesson: the opponent deck previews the next phase** | In a LATER phase of the first-run tutorial (a Durchlauf where no other hint is due), a once-per-profile banner points out that the opponent deck's card back shows which phase comes after the current Durchlauf (the opponent plays each Durchlauf with the deck of the COMING decision, `oppDeck` from `DECISION_SCHEDULE`; phase-color-coded backs: green Aufstellung, blue Architekt, orange Perk, purple Skill, diamond Legendär). Proposed DE (pending owner ok): "Wirf einen Blick auf das Gegnerdeck: Sein Rücken zeigt dir, welche Phase nach diesem Durchlauf kommt." Opponent deck gets the tutorial glow. "Mehr dazu" opens a NEW lesson (two pages, archetype-lesson pattern): page 1 shows the five real card backs with their phase names; page 2 shows the full run overview of which Durchlauf carries which phase (generated from `DECISION_SCHEDULE`/`LEG_PHASE_CYCLE`/`MAX_CYCLES`, never typed in). Mockup APPROVED (owner: "passt"; revision: skill deck back updated to the current dev asset): https://claude.ai/code/artifact/df88b8b8-0473-4820-93e9-fa631c490ca3 — hint timing Durchlauf 6, banner wording and grid as mocked (letters S/P/A/B/★). |
| Y2 | **Buildings toggle in the Aufstellung defaults to OFF + later hint** | The architect-buildings overlay toggle in the formation phase (`showArch` in FormationPhase.jsx, `ArchToggle`) currently defaults to ON (`options.archShowBuildings !== false`, remembered via options, #278). Owner: default becomes OFF (read flips to `=== true`; a player's explicit stored choice keeps winning in both directions). In a LATER first-run tutorial phase, a once-per-profile banner with the tutorial glow frame on the toggle points out that it can be switched on to see your buildings. Trigger proposal: the first formation phase in which at least one building stands (on the normal first-run schedule that is the Aufstellung before Durchlauf 7; hidden while `hasArch` is false). Proposed DE (pending owner ok): "Deine Gebäude sind hier ausgeblendet. Mit dem Schalter blendest du sie ein und siehst, welche Karten sie verstärken." Needs a `data-hint-anchor` on the ArchToggle row. |
| Y3 | **Order phase: letter legend instead of category color chips (screenshot)** | The chips next to the buildings toggle (Value blue · Score green · Formation orange, `ArchToggle`/`archCatList`) collide visually with the card frames of the same hues (frame color = NUMBER of formations: 1 green · 2 blue · 3 purple · 4 gold) — two color systems in one row. Owner: replace them with a letter legend describing the formation abbreviations the cards already carry (source `formationAbbr`, guard-checked single chars; DE W Wiederholung · F Farbblock · T Treppe · Z Wechsel, EN R Repeat · B Suit block · S Stair · Z Zigzag; role letters A/N/K/G resp. A/E/C/X). Mockup published for refinement (before/after + optional role-letter variant): https://claude.ai/code/artifact/76d6129b-71c6-4eea-8962-21a9a5225d9a — open points: legend always visible or only with the overlay on; role letters dynamically (only when present on the board, recommended) or omitted; the category-color explanation moves into the collapsible reference legend. |
