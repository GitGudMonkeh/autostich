...
| Q7 | **NEW hint: first ionized card explains the storm bars** | Once per profile, when the FIRST card gets ionized (first ion stack in the deck): an event card explaining Sturmgröße and Sturmintensität, anchored on the Blitz panel (`faction-lightning` glow/spotlight). Proposed DE (pending owner ok, dash-free): "Deine erste Karte ist ionisiert. Sturmgröße zählt, wie viele Karten deines Decks ionisiert sind, Sturmintensität, wie viele davon voll ionisiert sind. Volle Leisten schalten die Boni daneben frei." |
