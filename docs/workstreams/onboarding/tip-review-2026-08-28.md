...
## Round 4 (owner playtest after round 3) — COLLECTING
...
| V2 | **Eis page 3: Firn/Schnee instead of Einfrieren** | The eis section's third page ("Einfrieren", freeze rules) is replaced by a page explaining the snow system — in-game term: **Firn** (HUD: "Firn-Boden lädt" / "Firn-Reserve"; glossary entry "Firn-Boden" exists). Engine facts to carry: the reserve lies on the BOARD FIELD, not the card (firnStack, separate from glacierMass); open ground is charged by Skill Schneetreiben (win drifts +SCHNEETREIBEN_SEED to an open neighbour field, never under a glacier), Skill Dauerfrost (cycle end: open fields +NEAR/+FAR by distance to the nearest glacier) and the legendary Eiszeit flood; a card frozen onto a charged field starts at mass 0 and refills from its reserve at every cycle start up to full FIRN_REFILL_TARGET (=BURST_AT) mass — only the difference — until the reserve is empty; reserve is uncapped. The freeze rules of the old page survive in the tipps page (bullets 1 and 6), so nothing is lost. Draft page proposed in chat, pending owner ok. |
