...
| R22 | **New profile starts with 0 DP** | A fresh profile currently starts with 50 DP (`START_DECK_POINTS = 50` in storage.js, decision #316). Owner decision: a new profile starts with 0 DP → constant goes to 0. Separate and untouched unless the owner says otherwise: the one-time welcome bonus of {WELCOME_DP} = 50 DP after the first COMPLETED run (progression.js) — owner asked whether it stays. |
