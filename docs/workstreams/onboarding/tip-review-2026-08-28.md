...
| Q7 | **NEW hint: first ionized card explains the storm bars** | Once per profile, when the FIRST card gets ionized (first ion stack in the deck): an event card explaining Sturmgröße and Sturmintensität, anchored on the Blitz panel (`faction-lightning` glow/spotlight). Owner addition: also explain the threshold effects. Revised proposed DE (pending final ok): "Deine erste Karte ist ionisiert. Sturmgröße zählt die ionisierten Karten deines Decks: Ist die Leiste voll, bekommt jede Karte +{v} Wert. Sturmintensität zählt die voll ionisierten: Ist sie voll, wandelt der Überschlag Crit-Chance doppelt so schnell in Ladung um." ({v} = ION_SATURATION_VALUE.) |
