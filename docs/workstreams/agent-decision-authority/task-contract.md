...
**Two ranges, one terminology.** Both this contract and the handoff use these two names and no others.
...
the same way: the handoff was added by `175bfbf2` and updated by `d63e471a`, and both commits are
inside that range. A reviewer following the old definition would have excluded a file the range
contains. Scope compliance is proved over the whole history to the head either way, so the mistake
hid nothing — but it did make the two documents describe the same commits differently.
...
*Measured on Linux CI, most recently at the live-rule finalization commit* `a2357b47` *(run* `32579560825`*):*
...
| # | Finding | Disposition |
| --- | --- | --- |
| a | The handoff claimed six commits in the live-rule finalization range; git has seven. `d63e471a` was missing from a list written **while fixing the previous version of this same defect**. | **Fixed.** The count is now derived from `git log` and the list is complete. |
...
## Review round 4 — Codex, 2026-08-22

Verdict: **changes requested** on `ff144a5c`. The three live rule documents were confirmed
integration-ready with no new findings, and all four round-3 findings confirmed fixed. Two medium
findings remained, both in the review package.

| # | Finding | Disposition |
| --- | --- | --- |
| 1 | The reproduce command compared against the ambient `HEAD`. Because the review is performed without checking the branch out, `HEAD` is the reviewer's own branch; run as written, the command reported `DIFFERS` for all three live documents and appeared to refute a true claim. | **Fixed.** Both sides named explicitly, `HEAD` called out as the thing not to use, expected output stated. |
| 2 | This contract and the handoff defined the two ranges differently, and this contract's definition was wrong: it said the handoff was "not part of" a range that contains it. | **Fixed.** One terminology in both documents — *live-rule finalization range* and *package-record tail* — adopted from the reviewer's suggestion. |

Finding 1 is worth keeping in view beyond this task. The command was correct in the worktree where it
was written and wrong in the only environment it would ever be run in, because its author verified it
where they stood rather than where the reader stands. A reproduce command is only evidence if it runs
in the reader's context.

### Owner instruction, 2026-08-22 — closure review

The owner has scoped the next review to a **closure review**: the two findings above, plus any
regression their fixes could have caused. Already-closed findings stay closed; areas already
confirmed integration-ready are not re-reviewed; new findings are admissible only if this fix created
them or they are genuine blockers.
