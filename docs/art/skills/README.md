...
Hier liegen die **Master (1024 px, WebP q92)**. Die Auslieferungsfassung (384 px) entsteht erst beim
Einbau — nicht von Hand pflegen, sonst gibt es zwei Wahrheiten.

## Collection state (updated 2026-08-22)

| Archetype | Masters | Delivery copies | Per-lot light review |
|---|---|---|---|
| Lightning | Complete | Complete | Shipped as generated; owner-approved before this task |
| Ice | Complete | Complete | Shipped as generated; owner-approved before this task |
| Fire | Complete | Complete | Measured against the fire median; V3 decision recorded in the `icons-skills` visual review |
| Plant | Complete | Complete | Measured against the plant median; V3 decision recorded in the `icons-skills` visual review |

The delivery directories are generated by `scripts/skill-art-build.py`; the master files remain the
source of truth. Collection completeness is guarded against the live skill registry by
`test/skill-art.test.js`.
...
## Feuer — historische Erzeugungsnotizen zu den ersten sechs Mastern
...
Die übrigen Feuer-Master wurden am 22.08.2026 aus der auditierten Zuordnung ingestiert. Die folgenden
Notizen bleiben als Entstehungsprotokoll der ersten sechs Motive erhalten.
