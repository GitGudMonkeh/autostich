...
node sim/probes/kampagne-budget.mjs                      # how strong one campaign reward must be for the ladder to hold (§7)
N=24 node sim/probes/kampagne-hebel.mjs                  # what a single power lever is worth, paired against the same seeds (§8)
```
