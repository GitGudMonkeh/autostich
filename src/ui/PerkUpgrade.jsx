...
function UpgradeRow({ familyId, state, coins, onUpgrade, justRaised }) {
  ...
  // Der GANZE State — siehe SkillUpgrade: Preis und Deckel haengen daran, `{ coins }` schnitt beide ab.
  const buy = familyUpgradeBuy(state, tier);
  const fam = familyDef(familyId);
  if (!fam) return null;
  const cat = perkCat(fam.cat);
  const cur = fam.tiers[tier] || {};
  const zu = buy.maxed || buy.locked;          // wie beim Skill: blockiert ist blockiert
  const next = zu ? null : (fam.tiers[buy.next] || {});
  const d = next ? tierTextDiff(cur.desc || "", next.desc || "") : null;
  const tm = tierMeta(zu ? tier : buy.next) || { color: "#8a8a95" };
  const asks = !zu && needsTarget(fam, state.roles, familyId, buy.next);
  return (
    <button type="button" disabled={zu || !buy.can}
      onClick={zu || !buy.can ? undefined : () => onUpgrade({ id: familyId, name: fam.name, price: buy.price })}
      ...
      style={{ "--c": zu ? "#3a3850" : tm.color, opacity: zu ? 0.42 : (buy.can ? 1 : 0.62) }}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {!zu && <><TierArrow /><FamTierBadge tier={buy.next} /></>}
