...
export function GlacierPick({ state, onConfirm }) {
  const { deck = [], playerOrder = [], formations = [], roles = {}, glacierLocked = [], glacierMass = [] } = state;
  ...
  const disabledPos = glacierLocked.map((v, i) => (v ? i : -1)).filter((i) => i >= 0); // schon gefroren → nicht wählbar
  const ready = sel != null && !glacierLocked[sel];
  const pick = (pos) => { if (!glacierLocked[pos]) setSel(pos); };
  ...
  return (
    <div className="fixed inset-0 overlay-root z-30 flex items-center justify-center p-3" style={{ background: "#0c0c10ee", backdropFilter: "blur(2px)" }}>
      <div className="w-full max-w-4xl rounded-2xl p-5 max-h-[95dvh] overflow-y-auto overlay-card" style={phaseCard(PHASE_ACCENTS.ice)}>
