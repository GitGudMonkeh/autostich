...
export default function PrismaKaskadePixi({ panelRef, cardRef = null, trigger = 0,
  ...
  useEffect(() => {
    ...
    const flashTex = makeRadial([[0, 1], [0.5, 0.4], [1, 0]], RTX);
    ...
    function startPlay() { const a = appRef.current, pl = playRef.current; if (disposed) return; if (!a) { pl.pending = true; return; } pl.pending = false; pl.playing = true; pl.bt = 0; placer.invalidate(); st.current.onFire && st.current.onFire(); if (document.visibilityState !== "hidden") a.ticker.start(); }
