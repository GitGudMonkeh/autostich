import { Filter, GlProgram, UniformGroup, defaultFilterVert, Graphics } from "pixi.js";
...
/* Aurora als GPU-Shader (Pixi-Umbau) — reiner AMBIENT-Hintergrund: perspektivische Bögen zu je eigenem Fluchtpunkt,
   ...
   Umsetzung: ein Fullscreen-Fragment-Shader als Pixi-FILTER über einem bildschirmfüllenden Graphics-Rechteck (exakt
   0,0..W,H → die Filter-Fläche == das ganze Battlefield, `vTextureCoord` läuft sauber 0..1 → zentriert, vollflächig).
   (Der frühere Mesh-Ansatz mit Custom-GlProgram renderte auf der echten Pixi-Bühne nichts → zurück auf den Filter,
   der wie die eingebauten Pixi-Filter zuverlässig zeichnet.)
...
const FRAG = [
  ...
  "void main(){",
  "  vec2 uv = vec2(vTextureCoord.x, 1.0 - vTextureCoord.y);", // y spiegeln → Bögen unten (Filter-y läuft von oben)
  "  float t = uTime;",
  "  vec3 ac = vec3(0.0);",
  ...
  "    float env = smoothstep(0.0,0.05,uv.x)*smoothstep(1.0,0.95,uv.x);",         // volle Feldbreite abdecken
  "    float v = vshape * clump*(0.15 + 0.85*rays) * env;",
  "    float hcol = clamp(hAbove*2.1, 0.0, 1.0);",
  "    ac += auroraCol(hcol) * v;",
  "  }",
  "  vec3 rgb = ac * I_ * 1.7;",
  "  float a = clamp(max(rgb.r, max(rgb.g, rgb.b)), 0.0, 1.0) * 0.55;",           // ~ transparenter; Schwarz durchsichtig
...
export function createAuroraField(app) {
  ...
  // Bildschirmfüllendes Träger-Rechteck (exakt 0,0..W,H). Der Filter überschreibt seine Pixel; die Rechteck-Größe
  // definiert die Filter-Fläche → vTextureCoord 0..1 spannt das ganze Feld (zentriert, kein Versatz).
  ...
  function sizeTo() {
    ...
    lastW = W; lastH = H;
    target.clear();
    target.rect(0, 0, W, H).fill({ color: 0x000000, alpha: 1 });
  }
  sizeTo();
  ...
  function setParams(next) {
    ...
    target.visible = params.effect === "aurora";
