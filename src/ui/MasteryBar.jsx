...
import { masteryProgress, masteryGradeLabel, MASTERY_MAX_GRADE, MASTERY_MEISTER_MAX } from "../game/mastery.js";
...
export function MasteryBar({ grade = 0, score = 0 }) {
  ...
  const nextLabel = atMax ? null : masteryGradeLabel(g + 1); // „Rang II" / „Großmeister I" — driftsicher (#226)
  // Tier-Akzent: sobald das nächste/aktuelle Ziel im Großmeister-Band liegt → Gold statt Master-Lila.
  const gmTier = g >= MASTERY_MEISTER_MAX;
  const acc = gmTier ? "#d4a63a" : ACCENT;
  const accHi = gmTier ? "#e6c766" : ACCENT_HI;
    ...
    <div className="rounded-xl px-3 py-2 as-panel" style={{ background: "#17171c", border: `1px solid ${acc}44` }}
      ...
      <div className="relative h-2 rounded-full overflow-hidden" style={{ background: "#0e0e13" }}>
        <div className="absolute inset-y-0 left-0 rounded-full transition-[width] duration-500"
          style={{
            width: `${atMax ? 100 : pct}%`,
            background: `linear-gradient(90deg, ${acc}, ${accHi})`,
            boxShadow: atMax || pct > 92 ? `0 0 8px ${acc}aa` : "none",
          }} />
      </div>
    </div>
  );
