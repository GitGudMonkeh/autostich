...
▸ Once ~${pct(C.ION_SAT_BREADTH_FRAC)}% of the cards are ionized: all cards +${C.ION_SATURATION_VALUE} value.`,
  ...
  "ability.SK_FIRE_01.desc": `Wins with a margin give +${pct(C.EMBER_MULT - 1)}% more heat.`,
  ...
  "ability.SK_FIRE_06.desc": `All your cards gain trick value from heat: +${C.GLOWING_T1_VALUE} from ${C.GLOWING_T1_HEAT}%, +${C.GLOWING_T2_VALUE} from ${C.GLOWING_T2_HEAT}%, +${C.GLOWING_T3_VALUE} at ${C.GLOWING_T3_HEAT}%. The upper two also need a win at ${C.GLOWING_T2_MARGIN} resp. ${C.GLOWING_T3_MARGIN} margin within the current segment.`,
  "ability.SK_FIRE_07.name": "White Heat",
  "ability.SK_FIRE_07.desc": `Heat above ${C.HEAT_MAX}% banks as overheat, up to ${C.HEAT_MAX + C.OVERHEAT_MAX}%; the higher it stands, the less arrives. Every point gives +${pct(C.OVERHEAT_SCORE_STEP)}% fire score. It decays by ${C.OVERHEAT_DECAY} points per trick, ${C.OVERHEAT_DECAY_LOSS} on a loss.`,
  "ability.SK_FIRE_08.name": "Fire Roll",
  "ability.SK_FIRE_08.desc": `From ${C.FIREROLL_MIN_HEAT}% heat, every win in a row gives the next card +1 trick value (up to +${C.FIREROLL_MAX}). A loss resets it.`,
  "ability.SK_FIRE_09.name": "Combustion",
  "ability.SK_FIRE_09.desc": `A large margin gives more fire score: ×${num(C.VERBRENNUNG_T1_MULT)} from ${C.VERBRENNUNG_T1_MARGIN}, ×${num(C.VERBRENNUNG_T2_MULT)} from ${C.VERBRENNUNG_T2_MARGIN}.`,
  "ability.SK_FIRE_10.name": "Spark Flight",
  "ability.SK_FIRE_10.desc": `Every win below ${C.SPARKFLIGHT_MIN_MARGIN} margin puts ${num(C.SPARKFLIGHT_BANK_MULT)}× its fire score plus ${C.SPARKFLIGHT_FLOOR_BASE} into a store, +${C.SPARKFLIGHT_FLOOR_PER_SKILL} per further fire skill. A win from ${C.SPARKFLIGHT_MIN_MARGIN} margin pays it out as score, a loss halves it.`,
  "ability.SK_FIRE_11.name": "Wildfire",
  "ability.SK_FIRE_11.desc": `From ${C.CONFLAG_MIN_HEAT}% heat, the next win burns down to ${C.CONFLAG_KEEP}%: +${C.CONFLAG_PER_HEAT} score per burnt heat point, +${C.CONFLAG_PER_SKILL} per further fire skill (with ${C.SKILL_SLOTS} fire skills ≈ +${grp((C.HEAT_MAX - C.CONFLAG_KEEP) * (C.CONFLAG_PER_HEAT + C.CONFLAG_PER_SKILL * (C.SKILL_SLOTS - 1)))}).`,
  "ability.SK_FIRE_12.name": "Melting Point",
  "ability.SK_FIRE_12.desc": `Every win burns ${C.MELT_COST}% heat: ${C.MELT_SCORE_BASE} score per burnt point, +${num(C.MELT_SCORE_PER_HEAT)} per percent of heat held (${grp(Math.round(C.MELT_COST * (C.MELT_SCORE_BASE + C.MELT_SCORE_PER_HEAT * C.HEAT_MAX)))} per win at a full bar). Losses cost no heat.`,
  ...
  "ability.SK_FIRE_15.desc": `At the end of every cycle your lowest card each time permanently gains +${C.FORGE_VALUE} card value, as long as you have ${C.FORGE_COST} ash or more. Once the forge is full, further ash burns off as ash glow: +${grp(C.FORGE_OVERFLOW_SCORE)} score per ${C.FORGE_COST} ash.`,
  ...
  "ability.SK_FIRE_17.desc": `From ${C.SCHMELZOFEN_MIN_HEAT}% heat, brands additionally give −${C.SCHMELZOFEN_BRAND_BONUS} value and +${C.SCHMELZOFEN_BRAND_BONUS} ash. Forging costs ${pct(C.SCHMELZOFEN_FORGE_DISCOUNT)}% less ash.`,
  "ability.SK_FIRE_L01.name": "Sun Core",
  "ability.SK_FIRE_L01.desc": `Every win against a branded opponent card gives +${grp(C.SONNENKERN_BRAND_SCORE)} score per brand on it. If a cycle ends at ${C.SONNENKERN_MIN_HEAT}% heat or more, your brands stack up instead of being renewed (to ${C.SONNENKERN_BRAND_CAP} per card), and your cards below value ${C.SONNENKERN_CARD_CAP} permanently gain +${C.SONNENKERN_VALUE} card value.`,
  ...
  "ability.SK_FIRE_L03.desc": `Your entire win score is multiplied by the highest heat level you have ever reached: +${num(Math.round(C.SUNWRATH_PEAK_STEP * 1000) / 10)}% per percent up to ${C.HEAT_MAX}%, so up to ×${num(Math.round((1 + C.HEAT_MAX * C.SUNWRATH_PEAK_STEP) * 100) / 100)}, and +${num(Math.round(C.SUNWRATH_OVER_STEP * 1000) / 10)}% per point of overheat above that, with White Heat up to ×${num(Math.round((1 + C.HEAT_MAX * C.SUNWRATH_PEAK_STEP + C.OVERHEAT_MAX * C.SUNWRATH_OVER_STEP) * 100) / 100)}.`,
  "ability.SK_FIRE_L04.name": "Damascus Steel",
  "ability.SK_FIRE_L04.desc": `Forges your lowest card every cycle without ash (+${C.FORGE_VALUE} value, up to ${C.DAMASCUS_MAX_FORGED} cards). Forged cards fight with +${C.DAMASCUS_COMBAT} value. Every win gives +${C.DAMASCUS_PER_VALUE} score per point of forged value in your deck. One forging is ${C.FORGE_VALUE} points.`,
