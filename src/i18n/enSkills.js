...
▸ Once ~${pct(C.ION_SAT_BREADTH_FRAC)}% of the cards are ionized: all cards +${C.ION_SATURATION_VALUE} value.`,
  ...
  "ability.SK_FIRE_06.desc": `All your cards gain trick value from heat: +${C.GLOWING_T1_VALUE} from ${C.GLOWING_T1_HEAT}% heat. +${C.GLOWING_T2_VALUE} from ${C.GLOWING_T2_HEAT}% and +${C.GLOWING_T3_VALUE} at ${C.GLOWING_T3_HEAT}% have to be earned once per segment — with a win above a margin of ${C.GLOWING_T2_MARGIN} resp. ${C.GLOWING_T3_MARGIN}. A segment without such a trick drops you back.`,
  "ability.SK_FIRE_07.name": "White Heat",
  "ability.SK_FIRE_07.desc": `At full heat, the excess no longer goes to waste: it banks as overheat (up to ${C.HEAT_MAX + C.OVERHEAT_MAX}%) — the hotter you are, the less of it arrives. Every point of overheat gives +${pct(C.OVERHEAT_SCORE_STEP)}% on your entire fire score. It decays by ${C.OVERHEAT_DECAY} per trick (${C.OVERHEAT_DECAY_LOSS} on a loss) unless you keep feeding it.`,
  ...
  "ability.SK_FIRE_10.desc": `Every win below ${C.SPARKFLIGHT_MIN_MARGIN} margin puts ${num(C.SPARKFLIGHT_BANK_MULT)}× its fire score plus ${C.SPARKFLIGHT_FLOOR_BASE} (+${C.SPARKFLIGHT_FLOOR_PER_SKILL} per further fire skill) into a store. A win with a margin of ${C.SPARKFLIGHT_MIN_MARGIN} or more pays it out and empties it; a loss halves it.`,
  "ability.SK_FIRE_11.name": "Wildfire",
  "ability.SK_FIRE_11.desc": `From ${C.CONFLAG_MIN_HEAT}% heat, the next win burns your heat down to ${C.CONFLAG_KEEP}%: +${C.CONFLAG_PER_HEAT} score per burnt heat point, +${C.CONFLAG_PER_SKILL} per further fire skill (full ≈ +${grp((C.HEAT_MAX - C.CONFLAG_KEEP) * C.CONFLAG_PER_HEAT)}).`,
  "ability.SK_FIRE_12.name": "Melting Point",
  "ability.SK_FIRE_12.desc": `Before every trick −${C.MELT_COST}% heat; on a win ${C.MELT_SCORE_BASE} score per burnt point, +${num(C.MELT_SCORE_PER_HEAT)} per percent of heat you hold while doing it (${grp(Math.round(C.MELT_COST * (C.MELT_SCORE_BASE + C.MELT_SCORE_PER_HEAT * C.HEAT_MAX)))} score at a full bar).`,
