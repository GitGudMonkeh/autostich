...
const E = [
  ...
  ["crit", "Crítico", `Golpe crítico: la victoria cuenta con el multiplicador de crítico (base ×${num(C.CRIT_BASE_MULT)}). El crítico base es 0. La probabilidad de crítico viene de las familias de Precisión y de las habilidades de Rayo. Cada habilidad de Rayo sube además el multiplicador de crítico en +${num(C.LIGHTNING_CRIT_MULT_PER_SKILL)}×.`, ["crítico", "críticos", "golpe crítico", "golpes críticos"]],
  ...
  ["charge", "Carga", `Los críticos generan carga (máx. ${C.LIGHTNING_MAX_CHARGE}). Con la carga llena, los consumidores de rayo disparan su efecto y la gastan.`, ["carga", "cargas"]],
  ["ionize", "Ionización", `Una marca permanente en la carta: una carta ionizada da +${C.ION_SCORE_PER_STACK} de puntuación por acumulación al ganar y recibe después +1 acumulación (máx. ${C.ION_MAX_STACKS}). Además, el campo ionizado carga el aire: cada acumulación de ionización del mazo sube la probabilidad de crítico de TODAS las cartas ganadoras en +${pct(C.ION_CRIT_PP_PER_STACK)} % (en todo el campo, hasta +${pct(C.ION_CRIT_STACK_CAP * C.ION_CRIT_PP_PER_STACK)} %). Cuántas cartas se ionizan por consumo de carga crece con el arquetipo: +${C.ION_SPEED_PER_SKILL} por cada habilidad de Rayo por encima de ${C.ION_SPEED_MIN_SKILLS}.`, ["ionización", "ionizada", "ionizar", "ionizan"]],
  ["stapel", "Acumulación (ionización)", `Una carga de ionización sobre una sola carta (como máximo ${C.ION_MAX_STACKS} por carta). Cada acumulación da +${C.ION_SCORE_PER_STACK} de puntuación cuando esa carta gana y sube además la probabilidad de crítico en todo el campo (véase ionización). Una carta con ${C.ION_MAX_STACKS} acumulaciones está totalmente ionizada y desbloquea efectos especiales (entre ellos Cortocircuito y Perforación).`, ["acumulación de ionización", "acumulaciones de ionización", "acumulación", "acumulaciones"]],
  ["kaskade", "Cascada", "Un suceso enciende el siguiente. Con Rayo: un crítico sobre una carta ionizada o junto a ella genera carga adicional. Con Hielo: un glaciar que estalla arrastra a sus vecinos, de modo que una ola de estallidos recorre la agrupación.", ["cascada", "cascadas"]],
