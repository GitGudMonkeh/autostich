...
const E = [
  ...
  ["crit", "Crit", `Critical hit: the win counts with the crit multiplier (base ×${num(C.CRIT_BASE_MULT)}). Base crit is 0. Crit chance comes from the Precision families and from lightning skills (+${pct(C.LIGHTNING_CRIT_PER_SKILL)}% per lightning skill held). Crit chance above 100% slightly raises the crit multiplier.`, ["crit", "crits", "critical hit", "critical hits"]],
  ...
  ["ziehreihenfolge", "Draw order", "The fixed order in which your cards are played; they stay stable across cycles (only the opponent shuffles).", ["draw order", "card order"]],
  ...
  ["forge", "Forging", "Heat becomes permanent card value: at the end of the cycle the Forge gives your lowest card +3 value once the heat clears its threshold. Forged value stays in the card, even after a skill swap.", ["forging", "forged", "forge", "forged value"]],
  ...
  ["freeze", "Snow", `Snow sits as a reserve on the board cell, separate from the glacier's mass. Every cycle each open cell freezes +${num(G_GROUND)} snow and hands ${Number.isFinite(G_DRAW) ? `up to ${G_DRAW} of it` : "all of it"} to your glaciers — split across all of them, the closest one getting the largest share, none left empty. Freeze a glacier onto a charged cell and the accumulated snow becomes its ground reserve; it starts empty and draws from that every cycle back up to a full ${G_REFILL} mass (only the difference, never beyond), until the reserve is empty. Permafrost, Snowdrift and Ice Age charge open ground on top of that, never under a glacier.`, ["snow"]],
  ...
  ["gebaeude", "Building", "A placed building. It buffs the card sitting on its position in the trick. It never overlaps with others.", ["building", "buildings"]],
...
const out = {
  ...
  "glossary.cathint.frak": "The four resources, grouped by archetype.",
