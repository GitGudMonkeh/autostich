...
const FACE_R = 0x1b2942;            // right face — the lit side
const FACE_L = 0x0e1526;            // left face — in shadow
const FACE_T = 0x23334f;            // roof slab
const BUILD_MS = 2600;
...
function facadeGrid(g, f) {
  const { pt, i, j, k, rand, ground, win } = f;
  if (ground) {
    const q = quad(pt, i, j, k, 0.08, 0.92, 0.12, 0.74);
    g.poly(q).fill(GLASS);
    g.poly(q).fill({ color: win[1], alpha: 0.22 });
    g.moveTo(...pt(i, j, k, 0.02, 0.82)).lineTo(...pt(i, j, k, 0.98, 0.82))
      .stroke({ width: 1.4, color: win[1], alpha: 0.6 });      // entrance canopy
    return;
  }
  for (const [s0, s1] of [[0.12, 0.44], [0.56, 0.88]]) {
    const q = quad(pt, i, j, k, s0, s1, 0.24, 0.78);
    g.poly(q).fill(GLASS);
    const r = rand();
    if (r < 0.42) g.poly(q).fill({ color: rand() < 0.72 ? win[0] : win[1], alpha: 0.3 + rand() * 0.4 });
  }
  g.moveTo(...pt(i, j, k, 0, 0.08)).lineTo(...pt(i, j, k, 1, 0.08))
    .stroke({ width: 1, color: STEEL, alpha: 0.3 });           // floor band
}
...
function facadeRibbon(g, f) {
  const { pt, i, j, k, rand, ground, win } = f;
  const t0 = ground ? 0.14 : 0.3, t1 = ground ? 0.78 : 0.7;
  const q = quad(pt, i, j, k, 0.03, 0.97, t0, t1);
  g.poly(q).fill(GLASS);
  const r = rand();
  g.poly(q).fill({ color: r < 0.65 ? win[0] : win[1], alpha: 0.12 + r * 0.3 });
  g.moveTo(...pt(i, j, k, 0.03, t1)).lineTo(...pt(i, j, k, 0.97, t1))
    .stroke({ width: 1.2, color: win[1], alpha: 0.5 });        // bright lip of the band
  for (const s of [0.28, 0.52, 0.76]) {
    g.moveTo(...pt(i, j, k, s, t0)).lineTo(...pt(i, j, k, s, t1));
  }
  g.stroke({ width: 1, color: 0x060b16, alpha: 0.85 });        // mullions
...
function facadeExo(g, f) {
  ...
  const q = quad(pt, i, j, k, 0.17, 0.83, 0.17, 0.83);
  g.poly(q).fill(GLASS);
  if (rand() < 0.38) g.poly(q).fill({ color: rand() < 0.72 ? win[0] : win[1], alpha: 0.32 + rand() * 0.35 });
  g.poly(q).stroke({ width: 2, color: STEEL, alpha: 0.7 });
  if (k % 3 === 0) g.poly(quad(pt, i, j, k, 0, 1, 0, 0.11)).fill({ color: 0x2d4266, alpha: 1 });
  if ((i + j + k) % 5 === 0) {
...
function makePanel(def, variant, seed) {
  ...
  for (const b of bands) b.position.set(ox, oy);
  ...
  for (const f of sorted) {
    ...
    if (f.kind === "top") {
      ...
      if (f.k === maxK || rand() < 0.35) {                     // roof plant on some slabs
        const s = 0.3;
        g.poly([...P(f.i - s, f.j - s, f.k + 1.35), ...P(f.i + s, f.j - s, f.k + 1.35),
          ...P(f.i + s, f.j + s, f.k + 1.35), ...P(f.i - s, f.j + s, f.k + 1.35)])
          .fill(0x2c3f5e).stroke({ width: 1, color: STEEL, alpha: 0.6 });
      }
    } else {
      const pt = f.kind === "right" ? rPt : lPt;
      g.poly(quad(pt, f.i, f.j, f.k, 0, 1, 0, 1)).fill(f.kind === "right" ? FACE_R : FACE_L);
      variant.draw(g, { pt, i: f.i, j: f.j, k: f.k, rand, ground: f.k === minK, win });
    }
  ...
  c.addChild(loose, drift, scan);

  return {
    node: c, bands, cage, scan, drift, loose, oy, topLevel: maxK + 1,
...
async function main() {
  ...
  const layout = () => {
    ...
    panels.forEach((p, n) => {
      ...
      const s = Math.min(cw * 0.8 / p.bbox.w, (ROW_H - 96) * 0.95 / p.bbox.h, 2.2);
      p.node.scale.set(s);
      p.node.position.set(cx, top + 96 + (ROW_H - 96) / 2 - p.bbox.cy * s);
    });
  };
  layout();
