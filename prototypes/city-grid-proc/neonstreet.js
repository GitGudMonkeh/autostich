// Street study in the reference language — the same buildings, but separated by a street system.
// Deliberately OUTSIDE src/: design exploration.
//
// The buildings come from refRender.js unchanged. What is designed here is the ground between
// them, and it follows the same rule the facades do: the street is DARK and almost colourless,
// and everything you see on it is light that fell there. The asphalt is the darkest surface in
// the picture, the footway one step lighter, the kerb edge a thin bright line — and then pink and
// cyan pools where a shopfront faces the street. Markings are white and sparse; two of them are
// enough to say "road", twenty would say "diagram".
...
const WALK_H = 0.34;          // footway height in lattice units

/* ---- ground -------------------------------------------------------------------------------- */

const plate = (i0, i1, j0, j1, h) => [
...
function footway(g, i0, i1, j0, j1) {
...
function centreLine(g, along, at, from, to) {
...
function crossing(g, axis, at, from, to) {
...
function pool(glow, i, j, r, colour, strength = 1) {
  const [x, y] = P(i, j, 0);
  for (let n = 5; n >= 1; n--) {
...
function lamp(g, glow, i, j, h = 2.6) {
...
function car(g, glow, i, j, dir, colour = PAL.cyan) {
...
const A0 = -0.5, A1 = 8.5, B0 = 12.9, B1 = 21.9;             // block extents along i
const C0 = -0.5, C1 = 8.5, D0 = 12.9, D1 = 21.9;             // and along j
const MIDI = (A1 + B0) / 2, MIDJ = (C1 + D0) / 2;
...
function ground(g, glow) {
  // The slab: everything sits on it, and it is what the light pools onto.
  const m = 1.6;
...
async function main() {
  ...
  glow.blendMode = "add";
  const items = [
    ...BLOCKS.map((b) => ({ d: b.i0 + b.j0 + b.k0 * 0.01, run: () => drawBlock(body, glow, b, rng(Math.round(b.i0 * 733 + b.j0 * 977 + b.k0 * 31))) })),
